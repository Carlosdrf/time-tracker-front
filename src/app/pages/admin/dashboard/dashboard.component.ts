...
export class AdminDashboardComponent implements OnInit{
  ...
  constructor(private socketService: WebSocketService, private reports: ReportsService, private dashboardService: DashboardService, public customDate: CustomDatePipe, private fb: FormBuilder){
  }
  ...
  getUsers(){
    this.dashboardService.getUsers(this.searchForm.value).subscribe((v) => {
      this.users = v
      this.getEntries();
    })
  }
  getEntries(){
    ...
    this.dashboardService.getAllEntries(data).subscribe(
      next =>{
        this.entries = next
  ...
  async filterUsersData(entries: any, users: any){
    let acc: number
    let usersInfo = await users.map((element: any) =>{
      acc = 0
      ...
      let status = entries.filter((item:any)=> item.status === 0 && item.user_id === element.id)
      if(status.length > 0){
    ...
    this.usersInfo = this.usersInfo.map((element: any)=>{
      if(element.id === user_id[0]){
        if(user_id[1].status == 0 ){
          ...
          let timer = interval(1000).subscribe(()=>{
            element.started = this.customDate.getTotalHours(user_id[1].start_time)
          })
          this.subscription.push(timer)
        }else{
          element.status = null
