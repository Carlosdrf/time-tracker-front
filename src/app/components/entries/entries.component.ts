...
export class EntriesComponent implements OnInit{
  ...
  constructor(private socketService: WebSocketService, private dashboardService: DashboardService, private customDate: CustomDatePipe, private page: PagesComponent){ }
  ...
  updateTask(i: number, event: any){
    this.dashboardService.updateEntryTask(this.entries[i].task_id, this.entries[i]).subscribe(
      (v)=>{
        console.log(v)
  ...
  updateStart_time(date: Date, event: any, i: number){
    if(this.regex.test(event.target.value)){
      ...
      const data = this.setUpdateData(date, event.target.value, this.entries[i])
  ...
  public isToday(date: any){
    ...
    // console.log(date)
    const compareDate = this.customDate.transform(date.toLocaleString(), 'DD-MM-YYYY')
